...
Pre-configured non-root nginx, serving static or dynamic pages, APIs and assets, with additional development tools and defaults.
...
| source maps | `404` | **served** |
| `MAX_BODY_SIZE` | `16m` | **`64m`** |
...
In both modes: source maps are served, HTML is `Cache-Control: no-cache`, other assets get `STATIC_MAX_AGE`.
...
- *`CSP`*: Content Security Policy. Defaults to `default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self' ws: wss:; base-uri 'self'; form-action 'self'; frame-ancestors 'self'; object-src 'none'`.
- *`HSTS`*: `Strict-Transport-Security` value. Defaults to `""`: the header is not sent, so `localhost` is never pinned to HTTPS.
...
- *`CORS_ALLOWED_ORIGINS`*: regexp alternation of allowed origins. Defaults to `(.*)`: every origin is allowed, with credentials.
...
`CORS_ALLOWED_ORIGINS` is matched anchored against the whole `Origin`. To test the production value, escape the dots:
...
- *`MAX_BODY_SIZE`*: maximum request body. Defaults to `64m`.
- *`STATIC_MAX_AGE`*: `expires` for static assets. Defaults to `epoch`.
- *`PRECOMPRESSED_ASSETS_DIRECTORY`*: directory to pre-compress at start-up. Defaults to ``. Useless here: `gzip_static` and `brotli_static` are off.

Nothing is cached by default: `STATIC_MAX_AGE` is `epoch`, and so are the fixed-name files below, whatever `STATIC_MAX_AGE` is set to. An edited file shows up on the next reload:
...
`worker_rlimit_nofile` is `32768`: two descriptors per connection, with no `open_file_cache` here. As non-root, nginx cannot exceed the inherited hard limit. A low runtime limit logs `setrlimit(RLIMIT_NOFILE, 32768) failed` at start-up and stays in force.
